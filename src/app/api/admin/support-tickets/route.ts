...
import { AdminSupportTicketError, listAdminSupportTickets } from "@/lib/adminSupportTickets";
...
export async function GET(request: Request) {
  try {
    ...
    const result = await listAdminSupportTickets(
      {
        ...
        assignedAdminUid: searchParams.get("assignedAdminUid")?.trim() || undefined,
        requesterUid: searchParams.get("requesterUid")?.trim() || undefined,
        relatedEntityId: searchParams.get("relatedEntityId")?.trim() || undefined,
        relatedEntityType: (searchParams.get("relatedEntityType")?.trim() as "booking" | "provider" | "user" | null) || undefined,
    ...
    captureServerException(error, { route: "api/admin/support-tickets/route.ts" });
    return NextResponse.json(
      { error: "Nu am putut încărca tichetele de suport." },
      { status: 500 }
    );
  }
}
