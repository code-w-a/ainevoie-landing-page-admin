...
import { AdminReviewError, listAdminReviews } from "@/lib/adminReviews";
...
export async function GET(request: Request) {
  try {
    ...
    captureServerException(error, { route: "api/admin/reviews/route.ts" });
    return NextResponse.json(
      { error: "Nu am putut încărca review-urile." },
      { status: 500 }
    );
  }
}
